    ...
    <div style={{ background: theme.bg, minHeight: '100vh', transition: '0.4s', color: theme.text, fontFamily: 'sans-serif' }}>
      ...
      <header style={{ height: '80px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 5%', background: isDarkMode ? 'rgba(30, 41, 59, 0.85)' : '#ffffff', borderBottom: `1px solid ${theme.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <Link href="/profile" style={{ display: 'flex', alignItems: 'center', gap: '12px', background: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.04)', padding: '6px 16px 6px 6px', borderRadius: '50px', textDecoration: 'none' }}>
...
export default function CheckoutPage() {
  return (
    <Suspense fallback={<div style={{height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0f172a'}}>Loading...</div>}>
      <CheckoutContent />
    </Suspense>
  );
}
