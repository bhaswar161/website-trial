...
import NoticeBoard from '../components/NoticeBoard' // Import the new component
...
export default function NeetPage() {
  ...
  const toggleClassStatus = async (batchId: string, currentStatus: boolean) => {
    ...
    const { error } = await supabase.from('class_status').update({ is_live: !currentStatus }).eq('id', batchId);
    if (error) setLiveStatuses(prev => ({ ...prev, [batchId]: currentStatus }));
  };
  ...
  return (
    <div style={{ padding: '40px 5%', fontFamily: 'sans-serif', background: '#f5f7fb', minHeight: '100vh' }}>
      ...
      <div style={{ 
        display: 'grid', 
        gridTemplateColumns: '1fr 350px', // Batch cards on left, Notices on right
        gap: '30px',
        alignItems: 'start'
      }}>
