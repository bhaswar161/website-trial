...
export default function NeetPage() {
  ...
  useEffect(() => {
    setMounted(true)
    // Extract synced data from profile page storage
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .resource-card { flex: 1; minWidth: 240px; padding: 25px; border-radius: 20px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; border: 1px solid #f1f5f9; background: #fff; }
        .admin-btn { color: #ff4757; border: 2.5px solid #ff4757; padding: 8px 22px; border-radius: 14px; font-weight: 800; text-decoration: none; display: inline-block; }
      `}} />
      ...
      <header>
        ...
        <nav className="nav-center">
          <ul>
            ...
            {isOwner ? (
                <motion.div whileHover={{ scale: 1.05 }} animate={{ opacity: [0.9, 1, 0.9] }} transition={{ repeat: Infinity, duration: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 15, justifySelf: 'end' }}>
          <Link href="/profile" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ textAlign: 'left' }}>
                <div style={{ fontWeight: '900', fontSize: '15px', color: '#1c252e' }}>Hi, {displayName}</div>
                <div style={{ fontSize: '10px', color: '#5b6cfd', fontWeight: 'bold' }}>{isOwner ? 'FACULTY' : 'STUDENT'}</div>
            </div>
          </Link>
          <button onClick={() => signOut()} style={{ background: '#ff4757', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '12px', fontWeight: '700', cursor: 'pointer' }}>Logout</button>
        </div>
      </header>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px' }}>
        ...
        {/* PIC 1 ANIMATED RESOURCE BOXES */}
        <div style={{ display: 'flex', gap: '20px', marginBottom: '60px', flexWrap: 'wrap' }}>
            <motion.div whileHover={{ y: -8, scale: 1.02 }} className="resource-card" style={{ borderLeft: '6px solid #5b6cfd' }}>
                <div><b style={{fontSize:'18px'}}>Blogs</b><br/><small style={{color:'#666'}}>Read Our Latest Blogs</small></div>
                <span>➔</span>
            </motion.div>
            <motion.div whileHover={{ y: -8, scale: 1.02 }} className="resource-card" style={{ borderLeft: '6px solid #ff4ecd' }}>
                <div><b style={{fontSize:'18px'}}>PDF Bank</b><br/><small style={{color:'#666'}}>Access PDF Bank</small></div>
                <span>➔</span>
            </motion.div>
            <motion.div whileHover={{ y: -8, scale: 1.02 }} className="resource-card" style={{ borderLeft: '6px solid #10b981' }}>
                <div><b style={{fontSize:'18px'}}>Test Series</b><br/><small style={{color:'#666'}}>Practice with Our Mock Test</small></div>
                <span>➔</span>
            </motion.div>
            <motion.div whileHover={{ y: -8, scale: 1.02 }} className="resource-card" style={{ borderLeft: '6px solid #3b82f6' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: '35px' }}>
          {filteredBatches.map((batch) => {
            ...
            return (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={batch.id} style={{ background: '#fff', borderRadius: '24px', overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 10px 30px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ padding: '25px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '15px 0' }}>
                    ...
                    <div style={{ background: '#eefcf1', color: '#10b981', padding: '6px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: '800' }}>Discount of 100% applied</div>
                  </div>
