...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const dateInputRef = useRef<HTMLInputElement>(null); // Ref to trigger calendar
  ...
  const handleDoneDismiss = async (ev: any) => {
    ...
    await supabase.from('notices').insert([{ 
        ...
        content: `"${ev.title}" is now completed.` 
    }]);
    fetchData();
  };
          ...
          <div style={{ flex: 1 }}>
            ...
            {events.length === 0 ? <div style={emptyBox}>🕒 No upcoming events.</div> : events.map(ev => (
                <motion.div layout key={ev.id} style={dataRow}>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: '800', fontSize: '16px' }}>{ev.title}</div>
...
const drawerUI: any = { width:'420px', height:'100%', background:'#fff', display:'flex', flexDirection:'column', boxShadow: '-10px 0 30px rgba(0,0,0,0.05)', position: 'absolute', right: 0 };
