...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const handleSaveEvent = async () => {
    ...
    if (editingEvent) {
      ...
      await supabase.from('notices').insert([{ batch_id: batchId, title: "📅 Event Scheduled", content: `New: ${eventTitle} on ${new Date(eventDate).toLocaleString()}` }]);
    }
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} style={drawerUI} onClick={e => e.stopPropagation()}>
              <div style={drawerHeaderUI}>
                 ...
                 <div style={{display:'flex', gap:'10px'}}>
                    ...
                    <motion.button whileTap={{scale:0.95}} onClick={() => setShowNotifs(false)} style={{...markReadBtnUI, background:'#eee', color:'#333'}}>Close</motion.button>
                 </div>
              </div>
