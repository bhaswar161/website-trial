...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const fetchData = async () => {
    ...
    // Fetch Events (Simplified fetch to ensure visibility)
  ...
  const handleDismissDone = async (ev: any) => {
    ...
    await supabase.from('notices').insert([{ batch_id: batchId, title: "Notice: Event Finished", content: `Event "${ev.title}" is completed.` }]);
    fetchData();
  };
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={modalOverlay} onClick={() => setShowNotifs(false)}>
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} style={drawerUI} onClick={e => e.stopPropagation()}>
              <div style={drawerHeaderUI}>
                 ...
                 <div style={{display:'flex', gap:'10px'}}>
                    <button onClick={() => {setLastReadTime(Date.now()); localStorage.setItem(`last_read_${batchId}`, Date.now().toString())}} style={markReadBtnUI}>Read All</button>
