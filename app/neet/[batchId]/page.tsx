...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const handleSaveEvent = async () => {
    ...
    if (editingEvent) {
      ...
      const { data: newEv } = await supabase.from('events').insert([{ batch_id: batchId, title: eventTitle, event_time: eventDate, is_done: false }]).select().single();
      await supabase.from('notices').insert([{ batch_id: batchId, title: "📅 New Event", content: `Event: ${eventTitle} on ${new Date(eventDate).toLocaleString()}` }]);
    }
        ...
        <div style={headerInner}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            <div style={statPillGroup}>
                ...
                <motion.div whileHover={{ rotate: [0, -10, 10, -10, 0] }} style={bellContainer} onClick={() => setShowNotifs(true)}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={profileTrigger} onClick={() => setShowProfileMenu(!showProfileMenu)}>
                <motion.img whileHover={{ scale: 1.05 }} src={finalDisplayPic} style={navAvatar} />
        ...
        <div style={dashboardGrid}>
          <div style={{ flex: 1 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: '15px' }}>
              ...
              {isOwner && <motion.button whileHover={{ scale: 1.05 }} onClick={() => {setEditingEvent(null); setEventTitle(""); setShowEventModal(true)}} style={addBtn}>+ Create Event</motion.button>}
            </div>
            {events.map(ev => (
              <motion.div layout key={ev.id} style={{...dataRow, opacity: ev.is_done ? 0.5 : 1}}>
                ...
                {isOwner && (
                  <div style={{display:'flex', gap:'10px'}}>
                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => toggleEventDone(ev)} style={{...textActionBtn, color: ev.is_done ? '#22c55e' : '#5b6cfd'}}>
                        {ev.is_done ? '✓ Done' : 'Done?'}
                    </motion.button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={modalOverlay} onClick={() => setShowNotifs(false)}>
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} style={drawerUI} onClick={e => e.stopPropagation()}>
              <div style={drawerHeaderUI}>
                 ...
                 <div style={{display:'flex', gap:'10px'}}>
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={markAllRead} style={markReadBtnUI}>Mark All Read</motion.button>
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowNotifs(false)} style={{...markReadBtnUI, background:'#eee', color:'#333'}}>Close</motion.button>
                 </div>
              </div>
...
// --- STYLE DEFINITIONS ---
const headerWrapper: any = { position:'fixed', top:0, left:0, width:'100%', background:'#fff', borderBottom:'1px solid #f0f0f0', zIndex: 1000, height: '80px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' };
...
const addBtn: any = { background:'#6157ff', color:'#fff', border:'none', padding:'10px 20px', borderRadius:'12px', fontWeight:'bold', cursor:'pointer', boxShadow: '0 4px 15px rgba(97, 87, 255, 0.2)' };
...
const notifCardUI: any = { padding: '20px', borderRadius: '25px', background: '#f8fafc', border: '1px solid #f1f5f9', marginBottom: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' };
