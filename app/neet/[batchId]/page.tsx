  ...
  const handleSaveEvent = async () => {
    ...
    try {
        const newEv = { 
            ...
            is_done: false // This matches the SQL column we just added
        };
        ...
        alert("SQL Error: Please run the SQL command provided in the chat first! " + err.message);
    }
  };
      ...
      <main style={contentArea}>
          ...
          {['All Classes', 'All Tests', 'My Doubts', 'Community'].map((item, idx) => (
            <Link key={item} href={item === 'All Classes' ? `/neet/${batchId}/all-classes` : '#'} style={{ textDecoration: 'none' }}>
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}
                ...
                <span style={{ fontWeight: '800', color: '#000' }}>{item}</span>
        ...
        <div style={{ marginTop: '50px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: '25px' }}>
            ...
            {isOwner && <motion.button whileHover={{ scale: 1.05 }} onClick={() => setShowEventModal(true)} style={addBtn}>+ Create Event</motion.button>}
          </div>
            ...
            <motion.div layout key={ev.id} style={dataRow}>
              ...
              {isOwner && (
                <div style={{display:'flex', gap:'10px'}}>
                  ...
                  <button onClick={() => supabase.from('events').delete().eq('id', ev.id).then(()=>fetchData())} style={{...textActionBtnRed, background: '#fef2f2', padding: '8px 15px', borderRadius: '10px'}}>Delete</button>
                </div>
              )}
            </motion.div>
          ))}
        ...
        {showEventModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={modalOverlay} onClick={() => setShowEventModal(false)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} style={modal} onClick={e => e.stopPropagation()}>
               <h2 style={{fontWeight: '900', marginBottom: '10px'}}>Schedule Event</h2>
               <p style={{color: '#666', fontSize: '14px', marginBottom: '25px'}}>Auto-notifies all students.</p>
               
               <label style={inputLabel}>Event Title</label>
               <input placeholder="Ex: Physics Quiz" value={eventTitle} onChange={e => setEventTitle(e.target.value)} style={modalInput} />
               ...
               <div style={customDateWrapper} onClick={() => dateInputRef.current?.showPicker()}>
                  ...
                  <div style={dateDisplayFake}>
                     {eventDate ? new Date(eventDate).toLocaleString() : 'Select Date & Time'}
                     <span>📅</span>
                  </div>
               </div>

               <div style={{display: 'flex', gap: '12px', marginTop: '30px'}}>
                  <motion.button whileTap={{scale: 0.95}} onClick={handleSaveEvent} style={notifSendBtn}>Create Event</motion.button>
...
const batchBanner: any = { background:'#1c252e', color:'#fff', padding:'80px 60px', borderRadius:'30px', marginBottom:'50px', position: 'relative', overflow: 'hidden' };
...
const addBtn: any = { background:'#6157ff', color:'#fff', border:'none', padding:'12px 24px', borderRadius:'15px', fontWeight:'bold', cursor:'pointer' };
const modalOverlay: any = { position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.4)', zIndex:3000, display:'flex', alignItems:'center', justifyContent:'center', backdropFilter: 'blur(8px)' };
const drawerUI: any = { width:'420px', height:'100%', background:'#fff', display:'flex', flexDirection:'column', position: 'absolute', right: 0 };
const drawerHeaderUI: any = { padding:'30px 20px', borderBottom:'1px solid #eee', display:'flex', justifyContent:'space-between', alignItems: 'center' };
const markReadBtnUI: any = { background: '#f0f3ff', border: '1px solid #e0e7ff', padding: '10px 18px', borderRadius: '12px', fontSize: '13px', fontWeight: '700', color: '#5b6cfd' };
...
const subjectInputUI: any = { width:'100%', padding:'12px', borderRadius:'12px', border:'1px solid #eee', marginBottom:'10px' };
...
const modal: any = { background:'#fff', padding:'40px', borderRadius:'40px', width:'450px' };
const inputLabel: any = { display: 'block', fontSize: '13px', fontWeight: '800', color: '#888', marginBottom: '8px' };
const modalInput: any = { width:'100%', padding:'16px', borderRadius:'18px', border:'1.5px solid #eee', outline: 'none' };
...
const dateDisplayFake: any = { padding: '16px', borderRadius: '18px', border: '1.5px solid #eee', background: '#f9f9fb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#333' };
