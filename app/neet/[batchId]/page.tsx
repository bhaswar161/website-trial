...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const handleSaveEvent = async () => {
    ...
    if (editingEvent) {
      ...
      // AUTO-NOTIFY logic
      await supabase.from('notices').insert([{ batch_id: batchId, title: "📅 Event Scheduled", content: `A new event "${eventTitle}" has been added.` }]);
    }
...
const headerWrapper: any = { position:'fixed', top:0, left:0, width:'100%', background:'#fff', borderBottom:'1px solid #f0f0f0', zIndex: 1000, height: '80px', boxShadow:'0 2px 15px rgba(0,0,0,0.02)' };
...
const dataRow: any = { background:'#fff', padding:'25px', borderRadius:'30px', marginBottom:'12px', border:'1px solid #f0f0f0', display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow:'0 4px 12px rgba(0,0,0,0.02)' };
...
const notifInputUI: any = { width: '100%', padding: '15px', borderRadius: '15px', border: '1px solid #eee', minHeight: '80px' };
