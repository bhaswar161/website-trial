...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const handleSaveEvent = async () => {
    ...
    // NEW LOGIC: Create Event and Local UI State Push
    const newEvObj = { batch_id: batchId, title: eventTitle, event_time: eventDate, is_done: false };
    
    const { data: dbResult, error } = await supabase.from('events').insert([newEvObj]).select();
    ...
    if (!error) {
      ...
      // Dismiss modal and force UI refresh
  ...
  return (
    <div style={{ background: '#fcfdfe', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <main style={contentArea}>
        ...
        {/* UPCOMING EVENTS (CLEAN RESET SECTION) */}
