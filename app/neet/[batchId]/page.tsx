...
export default function BatchDashboard({ params }: any) {
  ...
  const handleSaveEvent = async () => {
    ...
    if (editingEvent) {
      ...
      // AUTO NOTIFICATION FOR NEW EVENT
  ...
  const handleDismissDone = async (ev: any) => {
    // 1. Mark as done (dismiss from list)
    await supabase.from('events').update({ is_done: true }).eq('id', ev.id);
    // 2. Automatically post a "Mission Accomplished" notification
    await supabase.from('notices').insert([{ batch_id: batchId, title: "Event Completed ✅", content: `The event "${ev.title}" is now finished.` }]);
    fetchData();
  };
      ...
      <main style={contentArea}>
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} style={batchBanner}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={studyGoalText}>Daily Progress: <b>{Math.floor(studySeconds/60)}m / 10m</b></div>
          </div>
        </motion.div>
