...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const finalDisplayName = session?.user?.name?.split(' ')[0] || "User";
  const finalDisplayPic = userProfile?.avatar_url || session?.user?.image || `https://ui-avatars.com/api/?name=${finalDisplayName}`;
...
// --- ALL STYLE DEFINITIONS (Kill Terminal Errors) ---
