...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const fetchData = async () => {
    ...
    // 3. Fetch Events (FORCED RAW FETCH)
    const { data: eData } = await supabase.from('events').select('*').eq('batch_id', batchId).order('event_time', { ascending: true });
    // This removes any strict filtering so you can actually see the rows
    if (eData) setEvents(eData.filter((ev: any) => !ev.is_done));
  };
  ...
  const handleSaveEvent = async () => {
    ...
    const eventObj = { batch_id: batchId, title: eventTitle, event_time: eventDate, is_done: false };
    ...
    await supabase.from('notices').insert([{ 
      ...
      content: `Upcoming: ${eventTitle} on ${new Date(eventDate).toLocaleString()}` 
    }]);

    // 3. FORCE UI UPDATE (Immediate push so it doesn't stay blank)
    setEvents(prev => [...prev, { ...eventObj, id: Math.random().toString() }]);
  ...
  const handleDismissDone = async (ev: any) => {
    ...
    await supabase.from('notices').insert([{ batch_id: batchId, title: "Task Completed ✅", content: `Event "${ev.title}" is finished.` }]);
    fetchData();
  };
      ...
      <main style={contentArea}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={batchBanner}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={studyGoalText}>Goal: 10 mins | Studied: <b>{Math.floor(studySeconds/60)}m</b></div>
          </div>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={modalOverlay} onClick={() => setShowStreakModal(false)}>
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} style={streakCardPremium} onClick={e => e.stopPropagation()}>
               <div style={{padding:'40px'}}>
                  <h2 style={{margin:0}}>Study Streak: {streak} Days</h2>
                  <div style={progressBarBg}><motion.div animate={{width:`${(studySeconds/600)*100}%`}} style={progressBarFill} /></div>
                  <button onClick={() => setShowStreakModal(false)} style={{...addBtn, marginTop:'20px', width:'100%'}}>Keep Going</button>
               </div>
            </motion.div>
          </motion.div>
        )}
        ...
        {showEventModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={modalOverlay} onClick={() => setShowEventModal(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} style={modal} onClick={e => e.stopPropagation()}>
               ...
               <button onClick={handleSaveEvent} style={{...notifSendBtn, marginTop:'20px'}}>Create & Notify</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
