...
export default function BatchDashboard({ params }: PageProps) {
  ...
  useEffect(() => { 
    ...
    const savedRead = localStorage.getItem(`last_read_${batchId}`);
    if (savedRead) setLastReadTime(parseInt(savedRead));
  ...
  return (
    <div style={{ background: '#fcfdfe', minHeight: '100vh', fontFamily: 'sans-serif' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={batchBanner}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={studyProgressBox}>
                Study Session: <b>{Math.floor(studySeconds/60)} / 10 mins</b>
            </div>
          </div>
        </motion.div>
      ...
      {/* MAGIC STREAK CARD MODAL */}
      <AnimatePresence>
        {showStreakModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={modalOverlay} onClick={() => setShowStreakModal(false)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} style={streakCardPremium} onClick={e => e.stopPropagation()}>
              ...
              {[...Array(6)].map((_, i) => (
                <motion.div
                  ...
                  style={{ position: 'absolute', top: `${Math.random()*100}%`, left: `${Math.random()*100}%`, fontSize: '20px' }}
                >⭐</motion.div>
              ))}

              <div ref={badgeRef} style={{position:'relative', zIndex: 2}}>
                <motion.div 
                    animate={{ background: ['linear-gradient(135deg, #5b6cfd, #a44ed1)', 'linear-gradient(135deg, #a44ed1, #5b6cfd)', 'linear-gradient(135deg, #5b6cfd, #a44ed1)'] }}
      ...
      {/* NOTIFICATION DRAWER (WITH OWNER PANEL) */}
      <AnimatePresence>
        {showNotifs && (
              ...
              <div style={{padding: '0 20px', flex: 1, overflowY: 'auto'}}>
                {notices.map(n => (
                    <div key={n.id} style={notifCardUI}>
                      <div style={{flex: 1}}>
                         <div style={notifTitleUI}>{n.title}</div>
...
const batchBanner: any = { background:'#1c252e', color:'#fff', padding:'80px 60px', borderRadius:'30px 30px 100px 30px', marginBottom:'50px', position: 'relative', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' };
...
const offeringItem: any = { padding:'35px', background:'#fff', borderRadius:'30px', border:'1px solid #f0f0f0', display:'flex', justifyContent:'space-between', alignItems:'center', cursor: 'pointer', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' };
...
const dataRow: any = { background:'#fff', padding:'25px', borderRadius:'30px', marginBottom:'12px', border:'1px solid #f0f0f0', display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' };
...
const streakHeaderBgPremium: any = { height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center' };
const streakCirclePremium: any = { width: '120px', height: '120px', background: '#fff', borderRadius: '50%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#5b6cfd', border: '6px solid rgba(255,255,255,0.3)', boxShadow: '0 10px 20px rgba(0,0,0,0.1)' };
...
const modalProgressFill: any = { height:'100%', background:'#5b6cfd' };
const shareSectionUI: any = { padding: '20px 20px 30px', background: '#f8fafc', borderTop: '1px solid #eee' };
