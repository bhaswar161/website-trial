...
export default function BatchDashboard({ params }: PageProps) {
  ...
  useEffect(() => { 
    // Sync with Homepage LocalStorage
              ...
              <motion.div key={ev.id} style={dataRow}>
                ...
                {isOwner && (
                    <div style={{display:'flex', gap:'10px'}}>
                        ...
                        <button onClick={() => deleteEvent(ev.id)} style={textActionBtnRed}>Delete</button>
                    </div>
                )}
              </motion.div>
            ))}
        ...
        {showStreakModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={modalOverlay} onClick={() => setShowStreakModal(false)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} style={streakCard} onClick={e => e.stopPropagation()}>
              ...
              <div ref={badgeRef} style={{background:'#fff', paddingBottom: '10px'}}>
              ...
              <div style={shareSectionUI}>
                <p style={{fontSize:'13px', color: '#666', marginBottom:'15px'}}>Share it with your friend by downloading it</p>
              ...
              {isOwner && (
                <div style={adminPanelNotifUI}>
                   ...
                   <div style={{display:'flex', gap:'10px', marginTop:'10px'}}>
                        ...
                        {editingNotif && <button onClick={() => {setEditingNotif(null); setNewNotice("")}} style={cancelTextBtn}>Cancel</button>}
                   </div>
                </div>
              )}
            </motion.div>
