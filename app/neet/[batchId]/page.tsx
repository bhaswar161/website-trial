...
export default function BatchDashboard({ params }: PageProps) {
  ...
  const handleDismissDone = async (ev: any) => {
    ...
    await supabase.from('notices').insert([{ batch_id: batchId, title: "Notice: Task Done", content: `The event "${ev.title}" has been completed.` }]);
    fetchData();
  };
